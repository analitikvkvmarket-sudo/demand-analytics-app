...
def _fetch_apps_script_matrix_snapshot(
    ...
    Failure of «Справочник» must never make the plan loader fall back
...
def _fetch_apps_script_entity_reference(
    ...
    """Read «Справочник» independently from the plan snapshot.
...
def load_all_shop_daily_totals(date_from: date, date_to_exclusive: date) -> pd.DataFrame:
    """Unfiltered raw totals for every shop_number in the selected period.

    This diagnostic intentionally does NOT use point_mapping and does NOT pass a
    list of shops into SQL. Therefore a new/mis-mapped point cannot disappear
    before we see the raw PostgreSQL result.
    """
    query = """
        SELECT
            business_date,
            ...
            SUM(net_quantity)::numeric AS sold_quantity,
            SUM(net_line_amount)::numeric AS revenue,
            COUNT(DISTINCT source_purchase_id)::bigint AS receipts,
    ...
    with pg_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                query,
                {"date_from": date_from, "date_to": date_to_exclusive},
            )
            records = cursor.fetchall()
            columns = [description.name for description in cursor.description]
    ...
    frame["business_date"] = pd.to_datetime(
        frame["business_date"], errors="coerce"
    ).dt.date
    ...
    frame["receipts"] = pd.to_numeric(frame["receipts"], errors="coerce").fillna(0).astype(int)
    frame["sku_count"] = pd.to_numeric(frame["sku_count"], errors="coerce").fillna(0).astype(int)
    return frame
...
def unmapped_classification_report_bytes(
    ...
    instruction = pd.DataFrame(
        {
            "Инструкция": [
                "Отчёт построен только по SKU, которых нет в листе «Справочник» матрицы 2.3.",
                ...
                "После подтверждения SKU добавьте его в лист «Справочник» матрицы 2.3 — приложение подхватит категорию и сущность автоматически.",
            ]
        }
    )
...
def prepare_analysis(
    ...
    # листа «Справочник». PostgreSQL остаётся источником факта продажи.
...
with st.sidebar:
    ...
    if st.button(
        ...
        for cached_loader in (
            load_sales, load_all_shop_daily_totals, load_sales_hourly, load_sales_timed,
        ...
        for state_key in (
            ...
            "raw_all_shops_daily_v751205", "raw_all_shops_signature_v751205",
...
def build_cycle_plan_v1(
    ...
    6. Основа расчёта — факт продаж найденной партии. День недели нового
       плана больше не влияет на коэффициент: если точный SKU или выбранное
       блюдо-основание по сущности полностью продалось в День 1, факт x1.5;
       во всех остальных случаях факт x1.0.
    """
    columns = [
        "Дата плана",
        "День недели",
        "Дата сравнения SKU",
        "Дата прошлого меню категории",
        "Точка",
        "SKU",
        "Название блюда",
        "Категория",
    ...
    for frame in (target, reference):
        ...
        frame["sku"] = frame["sku"].map(normalize_sku)
        frame["analyst_plan"] = pd.to_numeric(
            frame["analyst_plan"], errors="coerce"
        ).fillna(0.0).clip(lower=0)
    ...
    target = target[
        target["plan_date"].notna()
        & target["point_number"].notna()
        & target["sku"].notna()
    ].copy()
    ...
    if not reference.empty:
        reference = reference[
            reference["plan_date"].notna()
            & reference["point_number"].notna()
            & reference["sku"].notna()
        ].copy()
    ...
    for item in target.sort_values(
        ...
        # День недели не влияет на коэффициент.
        # ×1.5 применяется только если найденная партия полностью продалась
        # в первый день её жизненного цикла. Это одинаково работает и для
        # точного SKU, и для блюда-основания, найденного через сущность.
        if sold_out and completion_day == 1:
            sku_k = 1.50
            strength_priority = 4
            if entity_fallback_used:
                status = "Блюдо-основание по сущности продано полностью в День 1 · факт ×1.5"
            else:
                status = "SKU продан полностью в День 1 · факт ×1.5"
        elif sold_out:
            sku_k = 1.00
            strength_priority = 2
...
def export_cycle_plan_v1_excel(file_bytes: bytes, frame: pd.DataFrame) -> bytes:
        ...
        for source_row in sorted(unique_rows, reverse=True):
            ...
            for _, record in row_group.iterrows():
                ...
                if str(record.get("Статус", "") or "") in {"Проверить SKU", "Нет основания для расчёта"}:
                    ...
                    previous_plan = pd.to_numeric(
                        pd.Series([record.get("Прошлый план SKU")]),
                        errors="coerce",
                    ).iloc[0]
                    consumed = pd.to_numeric(
                        pd.Series([record.get("Факт SKU за срок")]),
                        errors="coerce",
                    ).iloc[0]
                    sku_k = pd.to_numeric(
                        pd.Series([record.get("K SKU")]),
                        errors="coerce",
                    ).iloc[0]
                    minimum = pd.to_numeric(
                        pd.Series([record.get("Минимум SKU")]),
                        errors="coerce",
                    ).iloc[0]
                    category_fact = pd.to_numeric(
                        pd.Series([record.get("Факт категории прошлой недели, шт.")]),
                        errors="coerce",
                    ).iloc[0]
                    minimum_sum = pd.to_numeric(
    ...
    for _, group in work.groupby(group_keys, dropna=False, sort=False):
        ...
        for _, record in group.sort_values("Точка", key=lambda s: s.map(_archive_point_sort_key)).iterrows():
            ...
            output_row[f"Прошлый план {point}"] = record.get("Прошлый план SKU")
        ...
        if product_abc_result.empty:
            ...
            if abc_filtered.empty:
                ...
                st.caption(
                    ...
                    "сущность берётся из актуального листа «Справочник» матрицы 2.3."
                )
...
if tab_cycle_plan.open:
    with tab_cycle_plan:
        ...
        st.caption(
            ...
            "2/3 — красный. Если надёжного основания нет, ячейка остаётся пустой. День недели на коэффициент не влияет: "
            "если найденный точный SKU или блюдо-основание по сущности полностью продалось в День 1, факт ×1,50; иначе факт ×1,00. "
                    ...
                    if selected_dates and st.button(
                        ...
                        try:
                            with st.spinner("Переношу факт партии -14 дней и применяю коэффициенты SKU…"):
                                ...
                                # Продажи нужны для всех партий сопоставимого плана: точной
                                # даты -14 и, при необходимости, сущности из другого дня той же недели.
                                loaded_reference_dates = (
                                    sorted(
                                        value
                                        for value in reference_plans.get(
                                            "plan_date", pd.Series(dtype=object)
                                        ).dropna().tolist()
                                        if isinstance(value, date)
                                    )
                                    if not reference_plans.empty
                                ...
                                sales_reference_dates = loaded_reference_dates or reference_dates
                                history_from = min(sales_reference_dates) + timedelta(days=1)
                                history_to = max(sales_reference_dates) + timedelta(
                                    days=max_lifecycle + 1
                                )
                                cycle_sales = load_forecast_history(
                                    history_from,
                                    history_to,
                                    points,
                                )
